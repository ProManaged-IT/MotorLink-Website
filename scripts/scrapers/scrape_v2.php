 ...
 *
 * Usage:
 *   php scripts/scrapers/scrape_v2.php --allow-google-places-cost  # Full run (billable Google Places)
 *   php scripts/scrapers/scrape_v2.php --refresh             # Re-discover (new cache)
 *   php scripts/scrapers/scrape_v2.php --enrich-only         # Phase 3 only
 *   php scripts/scrapers/scrape_v2.php --insert-only         # Phase 2 only
 *   php scripts/scrapers/scrape_v2.php --type=garage         # Single type only
 *   php scripts/scrapers/scrape_v2.php --enrich-limit=200    # Limit enrichment rows
 *   php scripts/scrapers/scrape_v2.php --insert-only --max-new=30 --new-per-type=10 --target-per-type=120
 *
...
// ── API KEY ──────────────────────────────────────────────────────────────────
$stmt = $db->prepare("SELECT setting_value FROM site_settings WHERE setting_key = 'google_places_scraper_api_key'");
$stmt->execute();
$apiKey = trim((string)($stmt->fetchColumn() ?: ''));
if (!$apiKey) {
    updateJob(['status' => 'error', 'error' => 'google_places_scraper_api_key is not configured']);
    die("ERROR: google_places_scraper_api_key is not configured. The public Google Maps key is intentionally excluded from scraping.\n");
}
jlog("Scraper API key loaded.");
...
function downloadPlacePhoto(string $photoRef, string $apiKey, string $slug): ?string
{
    ...
    global $projectRoot;
    $dir  = $projectRoot . '/uploads/business_logos';
